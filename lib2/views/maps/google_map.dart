...
class GoogleMap extends StatefulWidget {
  const GoogleMap({
    required this.initialLocation, super.key,
...
class _GoogleMapState extends State<GoogleMap> with WidgetsBindingObserver {
  ...
  void initState() {
    ...
    _loadMapStyles();
    Future.delayed(Duration.zero, _loadMapIcons);
  }
  ...
  Future<void> _loadMapStyles() async {
    ...
    final var mapStyles = <Brightness, String>{};
  ...
  Future<void> _loadMapIcons() async {
    if (widget.markers.isNotEmpty) {
      final mapIcons = <Brightness, Map<Enum, google.BitmapDescriptor>>{};

      final var size = MediaQuery.of(context).devicePixelRatio * 40;
      final markerGenerator = MarkerGenerator(size);

      //Generate walk icons
      for (final brightness in [Brightness.dark, Brightness.light]) {
        final icons = <Enum, google.BitmapDescriptor>{};

        for (final mapEnum in GoogleMapIcons.values) {
          final byteData =
              await Assets.asset.bytedata(brightness, mapEnum.logo);
          final image = await markerGenerator
              .fromByteData(byteData, mapEnum.color, mapEnum.color);
          icons[mapEnum] = image;
        }

        //Generate Places icons
        for (final Places placeEnum in Places.values) {
          final color =
  ...
  Set<google.Polyline> get _polylines {
    ...
    for (var i = 0; i < widget.paths.length; i++) {
      ...
      final polyline = google.Polyline(
