...
class _WalksViewState extends State<WalksView> with WidgetsBindingObserver {
  ...
  _retrievePosition() async {
    final bool useLocation = await PrefsProvider.prefs.getBoolean(Prefs.useLocation);
  ...
  _retrieveWalksHelper() async {
    ...
    final var newList = retrieveSortedWalks(_selectedDate,
  ...
  Future _retrieveWeathers(List<Walk> walks) async {
    final var weathers = <Future<List<Weather>>>[];
    for (var i = 0; i < math.min(walks.length, 5); i++) {
      final walk = walks[i];
      final var future = retrieveWeather(walk);
  ...
  Future<void> _firstLaunch() async {
    final bool firstLaunch = await PrefsProvider.prefs.getBoolean(Prefs.firstLaunch);
  ...
  Future<void> onFilterPressed() async {
    final bool useLocation = await PrefsProvider.prefs.getBoolean(Prefs.useLocation);
    if (!mounted) return;
    final newFilter = await Navigator.of(context).push<WalkFilter>(
        MaterialPageRoute(
            builder: (context) =>
