...
class _WalkDetailsViewState extends State<WalkDetailsView> {
  ...
  Future<List> _retrievePaths() {
    ...
    if (!widget.walk.isCancelled) {
      for (final Path path in widget.walk.paths) {
        if ((path.url?.isNotEmpty ?? false) && path.gpxPoints.isEmpty) {
          final Future<List<GpxPoint>> future = retrieveGpxPoints(path.url!);
  ...
  Widget build(BuildContext context) {
    final fullDate = DateFormat.yMMMMEEEEd('fr_BE');
    return Scaffold(
      key: scaffoldState,
      appBar: AppBar(
...
class __BottomSheet extends State<_BottomSheet> {
  ...
  List<Widget> get _children {
    ...
    const Widget header = ListTile(
      leading: Icon(Icons.route),
      title: Text('Les parcours'),
    );
    final paths = widget.walk.paths.reversed
