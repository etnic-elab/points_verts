...
class _WeatherSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (walk.weathers.isEmpty) {
      ...
      for (final Weather weather in walk.weathers) {
        final time = '${weather.timestamp.hour}h';
        final var temp = '${weather.temperature.round()}°';
        final windSpeed = '${weather.windSpeed.round()} km/h';
        widgets.add(Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: <Widget>[
            Text(
              time,
              semanticsLabel: "A ${time.replaceFirst('h', 'heure')}",
            ...
            Text(windSpeed,
                semanticsLabel:
                    "Vent : ${windSpeed.replaceFirst('/h', ' par heure')}",
                textScaler: const TextScaler.linear(0.8),),
          ],
        ),);
      }
...
class _RangesTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (walk.isWalk || walk.isOrientation) {
      final String title = Range.label(walk);
      final var subtitle =
...
class _GeoTile extends StatelessWidget {
  ...
  Widget? get geotext {
    ...
    return Text(text, semanticsLabel: text.replaceFirst('min.', 'minutes'));
  }
}
